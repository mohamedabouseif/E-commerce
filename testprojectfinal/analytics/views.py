	...
	def get(self, request, *args, **kwargs):
    	...
    	if request.GET.get('type') =='week':
				...
				for  x in range(0,days):
					...
					day_total = new_qs.totals_data()['totals__sum'] or 0
					salesItems.append(
						day_total
						) 
					#print(datetime_list)
			...
			if request.GET.get('type') =='4weeks':
					data['labels'] =  ["Four Week Ago", "Three Weeks Ago", "Two Weeks Ago", "Last Weeks", "This Week"]
					current = 5
					data['data'] = []
					for i in range(0, 5):
						new_qs = qs.by_weeks_range(weeks_ago=current, number_of_weeks=1)
						sales_total = new_qs.totals_data()['totals__sum'] or 0
...
class SalesView(LoginRequiredMixin, TemplateView):
    ...
    def get_context_data(self, *args, **kwargs):
    	...
    	start_date = timezone.now().date() - datetime.timedelta(hours=12)
