...
def test(request):
    marks = Category.objects.all()
    return render(request, 'mark.html', {'marks': marks})
def board_topics(request, pk):
    board = get_object_or_404(Category, pk=pk)
    return render(request, 'models.html', {'board': board})
def ProductCustomList(request,pk):
    board=get_object_or_404(Car, pk=pk)
    return render(request,'cars.html',{'board': board})
